...
const filePath = `Queries/${database}/Table_Column_Name.json`;
...
async function compareTablesFromQueries(queries) {
    for (const { pgQuery, sqlQuery } of queries) {
        await compareTableAndColumnNames(pgQuery, sqlQuery);
    }
}

async function compareTableAndColumnNames(pgQuery, sqlQuery) {
    ...
    try {
        ...
        for (const tableName of commonTableNames) {
            ...
            const sfColumns = sqlData.filter(row => row.table_name === tableName).map(row => row.column_name);
