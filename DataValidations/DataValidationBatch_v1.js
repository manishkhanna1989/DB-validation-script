...
const { fail } = require('assert');const logStream = fs.createWriteStream(logFilePath, { flags: 'a' });
let resultStatus = '';
let timestamp ; 
const filePath = 'Queries/Test.json';
const csvPath = `Result_csv/${database}_Data_Validation_Batch_v1.csv`;
...
async function compareDataForTable(tableName, database, pgQueryTemplate, sfQueryTemplate, totalRecords ) {
    try {
        ...
        while (offset < totalRecords) {
            ...
            // const sfQuery = `SELECT * FROM dbo.extractspnl WHERE cobdate = CAST('2024-06-14' AS DATE) ORDER BY BHID DESC OFFSET ${offset} ROWS FETCH NEXT ${batchSize} ROWS ONLY`;
            // Fetch batch from PostgreSQL
            // const pgQuery = `SELECT * FROM dbo.extractspnl WHERE cobdate = DATE '2024-06-14' ORDER BY BHID desc OFFSET ${offset} rows FETCH NEXT ${batchSize} ROWS ONLY;`
            const pgQuery = pgQueryTemplate.replace('${date}', date).replace('${offset}', offset).replace('${batchSize}', batchSize);             
            const sfQuery = sfQueryTemplate.replace('${date}', date).replace('${offset}', offset).replace('${batchSize}', batchSize);       
...
async function compareTablesFromQueries(queries) {
    for (const { tableName, pgQuery, sfQuery, parameters } of queries) {
        await compareDataForTable(tableName, database, pgQuery, sfQuery, parameters.totalRecords);
    }
}
...
async function compareTableData(tableName, pgData, sfData) {
    ...
    if (resultStatus === 'Fail') {
        if (sfData.length !== pgData.length) {
            ...
            comment = `Data for table ${tableName} does not match.`;
        }
    }else{
        comment = `Data for table ${tableName} matches between PostgreSQL and Sql.`;
    }
...
async function main() {
    try {
        // const filePath = 'Queries/FundAccounting1.json'; // Specify the path to your JSON file
...
main();
// compareDataForTable(tableName, database);