...
// const database = 'EmailScanner';
const logFilePath = `Log/${database}_log.txt`;
const logStream = fs.createWriteStream(logFilePath, { flags: 'a' });
let resultStatus = '';
let timestamp;
const filePath = 'Queries/EmailScanner.json';
const csvPath = `Result_csv/${database}_Data_Validation.csv`;

const normalizeDateTimeColumn =
[ 'rundate','murexendtime','modifiedat','createdat','updatedat','approvedat','validfrom','validto','date','adjustmentdate','fxdate','closedate','expirydate','cobdate','lastmodified','generateddate','reportdate','publisheddate','statusmodifiedat'];
const columnsToRound = ['financedtd','fxrate','pnlmtd','dtdreturn','ytdreturn','mtdreturn','pnlytd','pnldtd','adjustmentdtd','adjustmentmtd','adjustmentytd','financeytd'];
...
function compareDataRows(pgRow, sqlRow) {
    const differences = [];
    const expectedTimeDifferenceMs = [19800000, 0, 19799999, 3600000]; // Adjust this value as needed 5hrs:30min 
...
async function compareDataForTable(tableName, database, pgQuery,sfQuery ) {
    try {
...
async function compareTablesFromQueries(queries) {
    for (const { tableName, pgQuery, sfQuery } of queries) {
        await compareDataForTable(tableName, database, pgQuery, sfQuery);
    }
}
