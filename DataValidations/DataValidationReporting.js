...
let resultStatus = '';
const filePath = 'Queries/Reporting.json';
const csvPath = `Result_csv/${database}Data_Validation_Prod.csv`;

const normalizeDateTimeColumn =
[ 'createdat','updatedat', 'approvedat','rundate', 'cobdate', 'murexendtime','modifiedat', 'validfrom','validto','date','lastmodified','generateddate','reportdate','publisheddate','statusmodifiedat'];
const columnsToRound = ['pnlytd'];
...
function compareDataRows(pgRow, sqlRow) {
    const differences = [];
    const expectedTimeDifferenceMs = [19800000, 0, 19799999, 23399999]; // Adjust this value as needed 5hrs:30min 
...
async function compareTableData(tableName, pgData, sfData) {
    ...
    // const sqlRows = sfData.map(row => normalizeDataRow(row));
    ...
    const comment = resultStatus === 'Pass' ? `Data for table ${tableName} matches between PostgreSQL and Sql.` : `Data for table ${tableName} does not match.`;
