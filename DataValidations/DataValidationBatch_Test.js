...
const logFilePath = `Log/${database}_Test.txt`;
const logStream = fs.createWriteStream(logFilePath, { flags: 'a' });
const filePath = `Queries/Batch.json`;
const csvPath = `Result_csv/${database}_Data_Validation_Batch_Test.csv`;
...
['effectivedate','unbumpedmaturitydate','underlyingeffectivedate', 'underlyingunbumpedeffectivedate','tradedate','date',
    ...
    'interestcalculationenddate','leg1rolldate','leg2rolldate','interestcalculationstartdate','eventdate','inv_manager_date','deletedat', 'fundclosedate', 'holiday','startdate','enddate','rundate','murexendtime','modifiedat','createdat','updatedat','approvedat','validfrom','validto','adjustmentdate','fxdate','closedate','expirydate','cobdate','lastmodified','generateddate','reportdate','publisheddate','statusmodifiedat'];
...
function compareDataRows(pgRow, sqlRow) {
    const differences = [];
    const expectedTimeDifferenceMs = [19800000, 0, 19799999,16200000]; // Adjust this value as needed 5hrs:30min 
...
async function compareDataForTable(tableName, database, pgQueryTemplate, sfQueryTemplate, batchSize, totalRecords ) {
    try {
        ...
        await compareTableData(tableName, pgData, sfData,offset, batchSize);
        offset += batchSize;
