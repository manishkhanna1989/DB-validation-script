...
const csvPath1 = `Log/${database}_Count_Validation.csv`;
...
const pgData = await pgDB.any(`
    SELECT DISTINCT c.table_name, c.table_schema, 
       c.table_catalog 
        ...
        WHERE c.table_schema IN ('reporting')
    ...
    const sqlTableNames = new Set(sqlData.map(row => ({ catalog: row.table_catalog, schema: row.table_schema, name: row.table_name })));
